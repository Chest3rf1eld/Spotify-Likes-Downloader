...
`artist_aliases.tsv` позволяет сопоставлять альтернативные написания или транслитерации с одним или несколькими каноническими именами артиста. Все канонические варианты используются как варианты поискового запроса; для имени выходного файла используется кириллический вариант (предпочтительно с `ё`), если он есть.
...
## Переопределения треков

`track_overrides.tsv` позволяет переопределять автоматически сгенерированные поисковые запросы для конкретного Spotify track URI. В одной строке можно указать несколько targets — они пробуются раньше автоматически сгенерированных запросов.
...
Каждый target может быть прямым URL, поисковым target'ом вида `ytsearch<N>:query` или обычной строкой запроса (она будет обработана как `ytsearch1:query`).
